...
# Make sure this points to your NEW Robust ConvNeXt
CONVNEXT_PATH = os.path.join(PROJECT_ROOT, 'models', 'wing_classifier_convnext.pth') 

DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# --- VOTING WEIGHTS ---
# We slightly boost EfficientNet (The specialist) to break the tie on Sample 1
W_RESNET = 0.15
W_EFFNET = 0.45
W_CONVNEXT = 0.40

# --- TEMPERATURE SCALING ---
# Lower temperature (< 1.0) makes models more confident (sharper)
# Higher temperature (> 1.0) makes them softer (smoother)
TEMP = 0.8 
...
def load_models():
    print("⏳ Loading The Trinity (Sharpened)...")
...
def get_prediction_data(img_pil, model, transform):
    ...
    with torch.no_grad():
        ...
        logits_n = model(input_normal)[0]
        logits_f = model(input_flipped)[0]
        
        # Apply TEMPERATURE SCALING
        # Dividing logits by T < 1.0 increases the gap between large and small values
        out_n = torch.nn.functional.softmax(logits_n / TEMP, dim=0)
        out_f = torch.nn.functional.softmax(logits_f / TEMP, dim=0)
...
def run_prediction(image_path, r_model, e_model, c_model):
    ...
    # Report
    print("-" * 60)
    print(f"   🤖 ResNet50:      {name_r} ({conf_r:.1f}%)")
    print(f"   🤖 EfficientNet:  {name_e} ({conf_e:.1f}%)")
    print(f"   🤖 ConvNeXt:      {name_c} ({conf_c:.1f}%)")
    print("-" * 60)
    
    icon = "✅" if winner == "C.nigripes" else "❌"
    print(f"   🏆 FINAL DECISION: {icon} {winner} ({conf_final.item()*100:.1f}%)")
