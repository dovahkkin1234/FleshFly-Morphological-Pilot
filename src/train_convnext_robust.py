...
def train_convnext_robust():
    print(f"🔧 SYSTEM CHECK: Training ROBUST ConvNeXt on {DEVICE}...")
    torch.backends.cudnn.benchmark = False
    os.makedirs(MODEL_DIR, exist_ok=True)

    # 1. ROBUST TRANSFORMS (The Key Upgrade)
    train_transforms = transforms.Compose([
        ...
        # Color/Noise Augmentation (Simulate Raw Data)
        transforms.ColorJitter(brightness=0.3, contrast=0.3, saturation=0.3),
        transforms.RandomGrayscale(p=0.2), # 20% chance to be black & white
        transforms.GaussianBlur(kernel_size=3, sigma=(0.1, 2.0)), # Simulate blurry photos
        
        transforms.ToTensor(),
        transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
    ])
    
    # Validation stays clean (to measure true accuracy)
    ...
    # Apply robust transforms ONLY to training data
    # (We hack the dataset class wrapper logic here for simplicity)
    ...
    print(f"\n✅ ROBUST CONVNEXT COMPLETE. Best Acc: {best_acc:.4f}")

if __name__ == "__main__":
    train_convnext_robust()